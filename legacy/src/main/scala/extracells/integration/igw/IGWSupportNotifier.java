...
public class IGWSupportNotifier {
    ...
    private String supportingMod = null;
    ...
    public IGWSupportNotifier() {
        if (FMLCommonHandler.instance().getSide() == Side.CLIENT && !Loader.isModLoaded("IGWMod")) {
            ...
            if (config.get(Configuration.CATEGORY_GENERAL,
                    "enable_missing_notification", true,
    ...
    private class ThreadDownloadIGW extends Thread {
        ...
        public void run() {
            try {
                ...
                if (Minecraft.getMinecraft().thePlayer != null) {
                    Minecraft.getMinecraft().thePlayer.addChatMessage(
                            new ChatComponentText(EnumChatFormatting.GREEN +
                                    "Successfully downloaded. Restart Minecraft to apply."));
                }
