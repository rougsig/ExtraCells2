  ...
  override fun doBusWork(): TickRateModulation {
    ...
    try {
      ...
      for (fluid in config) {
        ...
        val maxAmountToFill = dest.fill(side.opposite, fluidStack, false)
        if (maxAmountToFill > 0) {
          val aeFluidStack = AEApi.instance().storage().createFluidStack(fluidStack)
          val extractedAmount = inv.extractItems(aeFluidStack, Actionable.MODULATE, mySrc)
          if (extractedAmount != null) {
            dest.fill(side.opposite, extractedAmount.fluidStack, true)
            fluidToSend -= extractedAmount.fluidStack.amount
            worked = true
          }
        }
